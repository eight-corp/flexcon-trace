...
export function ShipmentHistory({ refreshKey }: { refreshKey: number }) {
  ...
  useEffect(() => {
    void supabase.from('flexcon_shipments').select('id, shipped_at, vehicle_no, note, flexcon_destinations(name), flexcon_shipment_items(lot_number), workers(worker_name)').order('shipped_at', { ascending: false }).limit(200)
  ...
  const filtered = useMemo(() => {
    ...
    return shipments.filter((item) => item.flexcon_destinations?.name.toLowerCase().includes(term) || item.vehicle_no?.toLowerCase().includes(term) || item.workers?.worker_name.toLowerCase().includes(term) || item.flexcon_shipment_items.some((detail) => detail.lot_number.includes(term)))
  }, [search, shipments])

  const exportCsv = () => {
    const rows = [['出荷日時', '納品先', '作業者', '車両番号・便名', 'ロット番号', '備考']]
    filtered.forEach((shipment) => shipment.flexcon_shipment_items.forEach((item) => rows.push([
      new Date(shipment.shipped_at).toLocaleString('ja-JP'), shipment.flexcon_destinations?.name ?? '', shipment.workers?.worker_name ?? '', shipment.vehicle_no ?? '', item.lot_number, shipment.note ?? '',
    ])))
  ...
  return (
    <div>
      <div className="page-heading"><h1>出荷履歴</h1><p>納品先、作業者、車両番号、6桁のロット番号で検索できます。</p></div>
        ...
        {filtered.map((shipment) => (
          <article className="shipment-item" key={shipment.id}>
            <div className="shipment-head">
              <div><strong>{shipment.flexcon_destinations?.name ?? '納品先不明'}</strong><small>{new Date(shipment.shipped_at).toLocaleString('ja-JP')} / {shipment.workers?.worker_name ?? '作業者不明'}</small>{shipment.vehicle_no && <small><Truck size={13} style={{ verticalAlign: -2, marginRight: 4 }} />{shipment.vehicle_no}</small>}</div>
