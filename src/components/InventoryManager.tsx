...
import { AlertTriangle, ArrowDown, ArrowDownToLine, ArrowRightLeft, ArrowUp, ArrowUpFromLine, Camera, FileUp, Filter, Plus, Save, Search, Trash2, Warehouse, X } from 'lucide-react'
...
type InventoryColumn = 'movementDate' | 'cropYear' | 'movementType' | 'settlementNo' | 'workerName' | 'producerName' | 'origin' | 'productName' | 'grade' | 'quantity' | 'unit' | 'purchasePrice' | 'movementFrom' | 'movementTo' | 'note'
...
function movementValue(movement: InventoryMovement, key: InventoryColumn, calendarMode: CalendarMode) {
  ...
  if (key === 'note') return movement.note ?? ''
...
export function InventoryManager({ view, workerId, workerName, canOperate, isAdmin }: Props) {
  ...
  return <div className={`inventory-page ${view === 'balance' ? 'inventory-page-balance' : ''}`}>
    <div className="page-heading"><p>{view === 'history' ? '検査対象のお米は【検査記録】から登録して下さい。' : view === 'statement-reader' ? '仕切書を撮影し、米穀明細と仕入価格を読み取って登録します。' : view === 'statement-list' ? '撮影して登録した仕切書の明細を確認・編集します。' : '倉庫ごとの現在庫を産地、名称、等級別に表示します。'}</p></div>
