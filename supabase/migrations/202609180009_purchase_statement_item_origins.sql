...
alter table public.flexcon_purchase_statement_master_values
  ...
  check (value_type in ('recipient', 'issuer', 'origin', 'product', 'package'));
...
begin
  ...
  insert into public.flexcon_purchase_statement_items (
    statement_id, line_no, crop_year, origin, product_name, package_type, quantity, unit, unit_price, amount
  )
  select
    v_statement_id,
    item.source_order::integer,
    nullif(item.value->>'crop_year', '')::integer,
    btrim(coalesce(item.value->>'origin', '')),
...
begin
  ...
  if p_value_type not in ('recipient', 'issuer', 'origin', 'product', 'package') then raise exception 'マスタの種類が不正です。'; end if;
