...
alter table public.flexcon_purchase_statement_lines
  drop constraint if exists flexcon_purchase_statement_lines_purchase_price_check;

alter table public.flexcon_purchase_statement_lines
  add constraint flexcon_purchase_statement_lines_purchase_price_check
...
begin
  ...
  loop
    ...
    if nullif(v_record->>'purchase_price', '') is not null and (v_record->>'purchase_price')::numeric < 0 then raise exception '仕入価格が不正な明細があります。'; end if;
  ...
  loop
    insert into public.flexcon_purchase_statement_lines (
      ...
      grade, quantity, unit, purchase_price, to_warehouse_id, to_warehouse_name
    )
    select
      v_batch_id,
      ...
      btrim(v_record->>'unit'),
      nullif(v_record->>'purchase_price', '')::numeric,
      warehouse.id,
