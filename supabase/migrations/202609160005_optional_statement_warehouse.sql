-- 仕切り書Excel取込では入庫先を未指定のまま登録できるようにします。
...
begin
  ...
  loop
    ...
    ) then raise exception '仕切り書№%の入庫先倉庫が利用できません。', v_record->>'settlement_no'; end if;
    if not exists (
      select 1 from public.flexcon_inspection_options
  ...
  left join public.flexcon_inspection_options as warehouse
    on warehouse.id = nullif(record.value->>'to_warehouse_id', '')::uuid
...
begin
  ...
  if p_to_warehouse_id is not null then
    ...
    if v_warehouse_name is null then raise exception '入庫先倉庫が利用できません。'; end if;
  end if;
